...
int ProcessHeaders(char *path)
{
	...
	while(fgets(buffer,1024,filein))
	{
		...
		if(findptr)
		{
			...
			#elif (_MSC_VER == 1950)
			strncpy(findptr, "\"VC2026_0\"\n", 13);
			//#elif ((_MSC_VER >= 1950) && (_MSC_VER < 1950))
			//#error Please update your Visual Studio 2026 to Update 0 before continuing.  If you have an expired MSDN subscription and cannot update your paid version of Visual Studio, you can still use the Community version to compile DXGL.
			//#error If this error persists after updating Visual Studio, try uninstalling older MSVC toolchains.*/
			#elif (_MSC_VER > 1950)
			#pragma message ("Detected a newer version of Visual Studio, compiling assuming 2026.0.")
			strncpy(findptr, "\"VC2026_0\"\n", 13);
			#else
