...
void EnableDarkModeForMainDialog(HWND hDialog)
{
	...
	MakeEditDark(GetDlgItem(hTabs[0], IDC_WINDOWCORNER));
	...
	MakeEditDark(GetDlgItem(hTabs[2], IDC_ASPECT3D));
	MakeEditDark(GetDlgItem(hTabs[2], IDC_LOWCOLORRENDER));
	...
	MakeCheckboxDark(GetDlgItem(hTabs[3], IDC_REMEMBERWINDOWPOS));
	MakeCheckboxDark(GetDlgItem(hTabs[3], IDC_REMEMBERWINDOWSIZE));
	MakeEditDark(GetDlgItem(hTabs[3], IDC_WINDOWX));
	MakeEditDark(GetDlgItem(hTabs[3], IDC_WINDOWY));
	MakeEditDark(GetDlgItem(hTabs[3], IDC_WINDOWWIDTH));
	MakeEditDark(GetDlgItem(hTabs[3], IDC_WINDOWHEIGHT));
	MakeCheckboxDark(GetDlgItem(hTabs[3], IDC_WINDOWMAXIMIZED));
...
void EnableDarkModeForTextureShaderTest(HWND hDialog)
{
	...
	MakeEditDark(GetDlgItem(hDialog, IDC_TEXCOLORKEY));
	MakeButtonDark(GetDlgItem(hDialog, IDC_SETTEXCOLORKEY));
	...
	MakeEditDark(GetDlgItem(hDialog, IDC_CARG1));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_CARG1INV));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_CARG1A));
	MakeEditDark(GetDlgItem(hDialog, IDC_CARG2));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_CARG2INV));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_CARG2A));
	MakeEditDark(GetDlgItem(hDialog, IDC_COLOROP));
	MakeEditDark(GetDlgItem(hDialog, IDC_AARG1));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_AARG1INV));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_AARG1A));
	MakeEditDark(GetDlgItem(hDialog, IDC_AARG2));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_AARG2INV));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_AARG2A));
	MakeEditDark(GetDlgItem(hDialog, IDC_ALPHAOP));
	...
	MakeGroupBoxDark(GetDlgItem(hDialog, IDC_GRPALPHA));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_ALPHABLEND));
	MakeEditDark(GetDlgItem(hDialog, IDC_SRCBLEND));
	MakeEditDark(GetDlgItem(hDialog, IDC_DESTBLEND));
	MakeEditDark(GetDlgItem(hDialog, IDC_ALPHAREF));
	MakeButtonDark(GetDlgItem(hDialog, IDC_SPINALPHAREF));
	MakeEditDark(GetDlgItem(hDialog, IDC_ALPHAFUNC));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_ALPHASTIPPLE));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_ALPHATEST));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_COLORKEY));
	MakeCheckboxDark(GetDlgItem(hDialog, IDC_COLORKEYBLEND));
	...
	MakeGroupBoxDark(GetDlgItem(hDialog, IDC_GRPSTIPPLE));
	MakeEditDark(GetDlgItem(hDialog, IDC_LINESTIPPLEPATTERN));
	MakeEditDark(GetDlgItem(hDialog, IDC_LINESTIPPLEREPEAT));
	MakeButtonDark(GetDlgItem(hDialog, IDC_SPINLINEREPEAT));
	MakeEditDark(GetDlgItem(hDialog, IDC_FILLSTIPPLETYPE));
	MakeEditDark(GetDlgItem(hDialog, IDC_FILLSTIPPLEFILE));
	MakeButtonDark(GetDlgItem(hDialog, IDC_FILLSTIPPLEBROWSE));
...
LRESULT CALLBACK DisplayTabCallback(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		case IDC_DARKMODE:
			...
			*dirty = TRUE;
			break;
		case IDC_WINDOWCORNER:
			cfg->WindowCorner = GetCombo(hWnd, IDC_WINDOWCORNER, &cfgmask->WindowCorner);
			EnableWindow(GetDlgItem(hDialog, IDC_APPLY), TRUE);
			*dirty = TRUE;
			break;
		case IDC_COLOR:
...
void RefreshControls(HWND hWnd)
{
	...
	if (current_app && !tristate)
	{
		...
		SendDlgItemMessage(hTabs[0], IDC_WINDOWCORNER, CB_ADDSTRING, 0, (LPARAM)strdefault);
	...
	else if (!current_app && tristate)
	{
		...
		SendDlgItemMessage(hTabs[0], IDC_WINDOWCORNER, CB_DELETESTRING,
			SendDlgItemMessage(hTabs[0], IDC_WINDOWCORNER, CB_FINDSTRING, -1, (LPARAM)strdefault), 0);
	...
	SetCombo(hTabs[0], IDC_WINDOWCORNER, cfg->WindowCorner, cfgmask->WindowCorner, tristate);
...
LRESULT CALLBACK DXGLCfgCallback(HWND hWnd, UINT Msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		*/
		...
		// Window corners
		_tcscpy(buffer, _T("System default"));
		SendDlgItemMessage(hTabs[0], IDC_WINDOWCORNER, CB_ADDSTRING, 0, (LPARAM)buffer);
		if (osver.dwBuildNumber >= 22000)
		{
			_tcscpy(buffer, _T("Square corners"));
			SendDlgItemMessage(hTabs[0], IDC_WINDOWCORNER, CB_ADDSTRING, 0, (LPARAM)buffer);
			_tcscpy(buffer, _T("Rounded corners"));
			SendDlgItemMessage(hTabs[0], IDC_WINDOWCORNER, CB_ADDSTRING, 0, (LPARAM)buffer);
			_tcscpy(buffer, _T("Small rounded corners"));
			SendDlgItemMessage(hTabs[0], IDC_WINDOWCORNER, CB_ADDSTRING, 0, (LPARAM)buffer);
			SendDlgItemMessage(hTabs[0], IDC_WINDOWCORNER, CB_SETCURSEL, cfg->WindowCorner, 0);
		}
		else
		{
			SendDlgItemMessage(hTabs[0], IDC_WINDOWCORNER, CB_SETCURSEL, 0, 0);
			EnableWindow(GetDlgItem(hTabs[0], IDC_WINDOWCORNER), FALSE);
		}
		// Paths
		EnableWindow(GetDlgItem(hTabs[3], IDC_PATHLABEL), FALSE);
		EnableWindow(GetDlgItem(hTabs[3], IDC_PROFILEPATH), FALSE);
