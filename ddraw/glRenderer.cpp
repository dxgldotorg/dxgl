	...
	case OP_BLT:  // Perform a Blt() operation, issuing necessary commands to set it
		...
		if (This->state.last_cmd.Generic.opcode == OP_BLT)
		{
			...
			if(!restart_cmd)
			{
				...
				for (i = 0; i < 6; i++)
					tmp_cmd.BltVertex_STORAGE.index[i] += (WORD)lastcmd->count;
	...
	case OP_DRAWSCREEN:  // Draws the screen.  Flip command buffer after executing.
		...
		if (cmd->DrawScreen.texture->levels[0].ddsd.ddsCaps.dwCaps & DDSCAPS_PRIMARYSURFACE)
		{
			...
			if (memcmp(&r2, &r1, sizeof(RECT)))
				SetWindowPos(This->RenderWnd->hWnd, NULL, 0, 0, r1.right, r1.bottom, SWP_SHOWWINDOW);
		}
		LONG sizes[6];
		GLfloat view[4];
		GLint viewport[4];
		...
		if (cmd->DrawScreen.texture->levels[0].ddsd.ddsCaps.dwCaps & DDSCAPS_PRIMARYSURFACE)
		{
			...
			else
			{
				...
				ClientToScreen(This->RenderWnd->hWnd, (LPPOINT)&r2.left);
				ClientToScreen(This->RenderWnd->hWnd, (LPPOINT)&r2.right);
